...
def split_set_cookie_header(header_value):
    if not header_value:
        return []
    cookies = []
    start = 0
    ...
    value_len = len(value)
    while i < value_len:
        if value[i:i + 8].lower() == "expires=":
            in_expires = True
            i += 8
            continue
        if in_expires and value[i] == ";":
            in_expires = False
        if value[i] == "," and not in_expires:
            cookie = value[start:i].strip()
            if cookie:
...
def extract_set_cookies(headers):
    ...
    return split_set_cookie_header(header_value)


def rewrite_set_cookie(cookie_value, is_secure):
    if not cookie_value:
        return cookie_value
    ...
    for part in parts:
        ...
        if lower.startswith("domain="):
            continue
        if not is_secure and lower == "secure":
            continue
        rewritten.append(part)
    return "; ".join(rewritten)


def apply_upstream_cookies(response, headers, is_secure):
    for cookie in extract_set_cookies(headers):
        rewritten = rewrite_set_cookie(cookie, is_secure)
        if rewritten:
...
async def chatgpt_reverse_proxy(request: Request, path: str):
    try:
        ...
        # Apply workspace headers from cookies only for API-like paths.
        if ("backend-api/" in path or "backend-alt/" in path or "public-api/" in path or "gizmos" in path):
            ws_id = request.cookies.get("ws_id")
            ws_org = request.cookies.get("ws_org")
            if ws_id and "openai-organization" not in [k.lower() for k in headers.keys()]:
                headers["openai-organization"] = ws_id
            if ws_org and "oai-organization-id" not in [k.lower() for k in headers.keys()]:
                headers["oai-organization-id"] = ws_org
        ...
        try:
            ...
            if r.status_code == 307 or r.status_code == 302 or r.status_code == 301:
                ...
                return apply_upstream_cookies(response, r.headers, is_secure)
